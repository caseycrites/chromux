...
function saveCurrentSession() {
  ...
  if (isUserSession() && sessionsIndex.indexOf(currentSession.name) < 0) {
    sessionsIndex.push(currentSession.name)
    sessionObj[sessionsIndexKey] = sessionsIndex
  }
...
function loadSessionsIndex() {
  return new Promise(function(resolve, reject) {
    chrome.storage.sync.get(sessionsIndexKey, function(obj) {
      if (chrome.runtime.lastError)
        reject()
      else {
        sessionsIndex = obj[sessionsIndexKey] || []
        resolve()
      }
    })
  })
...
function startup() {
  ...
  loadSessionsIndex().then(buildCurrentSession)
}
...
function isUserSession() {
  return currentSession.name.substring(0, userSessionsKey.length) === userSessionsKey
}
